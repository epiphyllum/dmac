...
DOCKER_EXE={{home}}/.dockerdata/bin/docker
...
case "$N" in
	...
	# If we didn't find anything, try to lookup the container with Docker.
        ...
        while [ $RETRIES -gt 0 ]; do
      	    DOCKERPID=$($DOCKER_EXE inspect --format='{{ .State.Pid }}' $GUESTNAME)
            [ $DOCKERPID != 0 ] && break
            sleep 1
            RETRIES=$((RETRIES - 1))
        done
...
[ $IFTYPE = phys ] && [ "$VLAN" ] && [ -d /sys/class/net/$IFNAME.VLAN ] && {
    [ -z "$(sudo ip -d link show $IFNAME.$VLAN | grep "vlan.*id $VLAN")" ] && {
...
[ -f /var/run/netns/$NSPID ] && sudo rm -f /var/run/netns/$NSPID
sudo ln -s /proc/$NSPID/ns/net /var/run/netns/$NSPID

# Check if we need to create a bridge.
[ $IFTYPE = bridge ] && [ ! -d /sys/class/net/$IFNAME ] && {
    [ $BRTYPE = linux ] && {
        (sudo ip link add dev $IFNAME type bridge > /dev/null 2>&1) || (sudo brctl addbr $IFNAME)
        sudo ip link set $IFNAME up
    }
    [ $BRTYPE = openvswitch ] && {
        sudo ovs-vsctl add-br $IFNAME
    }
}
...
[ $IFTYPE = bridge ] && {
    ...
    case "$BRTYPE" in
        linux)
            (sudo ip link set $LOCAL_IFNAME master $IFNAME > /dev/null 2>&1) || (sudo brctl addif $IFNAME $LOCAL_IFNAME)
            ;;
        openvswitch)
            sudo ovs-vsctl add-port $IFNAME $LOCAL_IFNAME ${VLAN:+"tag=$VLAN"}
            ;;
    esac
...
[ $IFTYPE = phys ] && {
    [ "$VLAN" ] && {
        [ ! -d /sys/class/net/$IFNAME.$VLAN ] && {
            sudo ip link add link $IFNAME name $IFNAME.$VLAN mtu $MTU type vlan id $VLAN
        }
    ...
    sudo ip link add link $IFNAME dev $GUEST_IFNAME mtu $MTU type macvlan mode bridge
    sudo ip link set $IFNAME up
}

sudo ip link set $GUEST_IFNAME netns $NSPID
sudo ip netns exec $NSPID ip link set $GUEST_IFNAME name $CONTAINER_IFNAME
[ "$MACADDR" ] && sudo ip netns exec $NSPID ip link set dev $CONTAINER_IFNAME address $MACADDR
if [ "$IPADDR" = "dhcp" ]
then
    [ $DHCP_CLIENT = "udhcpc"  ] && sudo ip netns exec $NSPID $DHCP_CLIENT -qi $CONTAINER_IFNAME -x hostname:$GUESTNAME
    if [ $DHCP_CLIENT = "dhclient"  ]
    then
        # kill dhclient after get ip address to prevent device be used after container close
        sudo ip netns exec $NSPID $DHCP_CLIENT -pf "/var/run/dhclient.$NSPID.pid" $CONTAINER_IFNAME
        kill "$(cat "/var/run/dhclient.$NSPID.pid")"
        rm "/var/run/dhclient.$NSPID.pid"
    fi
    [ $DHCP_CLIENT = "dhcpcd"  ] && sudo ip netns exec $NSPID $DHCP_CLIENT -q $CONTAINER_IFNAME -h $GUESTNAME
else
	...
	sudo ip netns exec $NSPID ip route get $GATEWAY >/dev/null 2>&1 || \
		sudo ip netns exec $NSPID ip route add $GATEWAY/32 dev $CONTAINER_IFNAME
...
then
    IPADDR=$(echo $IPADDR | cut -d/ -f1)
    sudo ip netns exec $NSPID arping -c 1 -A -I $CONTAINER_IFNAME $IPADDR > /dev/null 2>&1 || true
else
